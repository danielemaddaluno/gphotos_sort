...
class Album extends Gui{
    ...
    async parseAlbum(){  // これでやっとアルバム情報揃う
        try{
            ...
            script_src = script_src.replace(');</script>', '');
            // console.log(script_src);
            script_src = script_src.split(/data: */)[1].split(/, *sideChannel: */)[0];
            // console.log(script_src);
            const stat_j = JSON.parse(script_src);
            console.log('stat_j: ');
            console.log(stat_j);
            this.imgs = stat_j[1];
